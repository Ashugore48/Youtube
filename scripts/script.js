...
const API_KEY = "AIzaSyDT_XWbC8fXIoOHOvzEDM0FjcAcxq-SorM";

const container = document.getElementById("videos-container");

async function getVideo(q) {
  const url = `${BASE_URL}/search?key=${API_KEY}&q=${q}&type=video&videoDuration=medium&maxResults=20`;
...
function renderVideos(videos) {
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    if (video.statistics.viewCount >= 1000000) {
      updatediews = "M";
      video.statistics.viewCount = Math.floor(
    ...
    container.innerHTML += `<div class="video-info">
        <div class="video-image" onclick="openVideoDetails('${
          video.id
        }')">
            <img src="${
        ...
        <div class="video-description">
            ...
            <div class="channel-description">
                <p class="main-title">${video.snippet.localized.title}</p>
